 ...
 *    [ILocalSocketManager] that will receive callbacks from the server including
...
class AmSocketServer {
    ...
    companion object {
        const val LOG_TAG: String = "AmSocketServer"
        ...
        fun processAmClient(
            ...
            val amCommandList = ArrayList<String>()
        ...
        fun sanitizeExitCode(clientSocket: LocalClientSocket, exitCode: Int): Int {
            ...
            if (sanitizedExitCode < 0 || sanitizedExitCode > 255) {
                Logger.logWarn(
                    LOG_TAG,
                    "Ignoring invalid peer " + clientSocket.getPeerCred().getMinimalString() + " result value \"" + sanitizedExitCode + "\" and force setting it to \"" + 1 + "\""
                )
                sanitizedExitCode = 1
            }
         ...
         * @param amCommandList The [List] to set list of arguments in.
         ...
         * @param checkDisplayOverAppsPermission Check if [android.Manifest.permission.SYSTEM_ALERT_WINDOW]
        ...
        fun runAmCommand(
            ...
            try {
                    ...
                    PrintStream(stdoutByteStream).use { stdoutPrintStream ->
                        ByteArrayOutputStream().use { stderrByteStream ->
                            PrintStream(stderrByteStream).use { stderrPrintStream ->
                                if (checkDisplayOverAppsPermission && amCommandArray != null && amCommandArray.isNotEmpty() &&
                ...
                return AmSocketServerErrno.ERRNO_RUN_AM_COMMAND_FAILED_WITH_EXCEPTION.getError(
                    e,
                    Arrays.toString(amCommandArray),
                    e.message
                )
            }
