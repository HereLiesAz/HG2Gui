...
object PackageUtils {
    ...
    fun getContextForPackage(context: Context, packageName: String?, flags: Int): Context? {
        if (packageName == null) {
            Logger.logVerbose(LOG_TAG, "Failed to get null package context with flags $flags: packageName is null")
            return null
        }
        return try {
    ...
    fun getContextForPackageOrExitApp(
        ...
        if (packageContext == null && exitAppOnError) {
            var errorMessage = context.getString(
                R.string.error_get_package_context_failed_message,
                packageName
            )
            if (!DataUtils.isNullOrEmpty(helpUrl)) {
                errorMessage += "\n" + context.getString(
                    R.string.error_get_package_context_failed_help_url_message,
                    helpUrl
                )
            }
     ...
     * Get the [PackageInfo] for the package associated with the `packageName`.
     *
     * Also check [isAppInstalled] if targetting targeting sdk
     * `30` (android `11`) since [PackageManager.NameNotFoundException] may be thrown.
     *
     * @param context The [Context] for operations.
     * @param packageName The package name of the package.
     * @param flags The flags to pass to [PackageManager.getPackageInfo].
    ...
    /**
     * Get the `privateFlags` [java.lang.reflect.Field] of the [ApplicationInfo] class.
     *
    ...
    /**
     * Get the `seInfo` [java.lang.reflect.Field] of the [ApplicationInfo] class.
     *
    ...
    fun getApplicationInfoSeInfoForPackage(applicationInfo: ApplicationInfo): String? {
        ...
        return try {
            ReflectionUtils.invokeField(
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) "seinfo" else "seInfo",
                applicationInfo
            ).value as? String
        } catch (e: Exception) {
            // ClassCastException may be thrown
    ...
    /**
     * Get the `seInfoUser` [java.lang.reflect.Field] of the [ApplicationInfo] class.
     *
    ...
    fun isApplicationInfoPrivateFlagSetForPackage(flagToCheckName: String, applicationInfo: ApplicationInfo): Boolean? {
        ...
        return (privateFlags and flagToCheck) != 0
    }
    ...
    /**
     * Check if the app associated with the `context` has [ApplicationInfo.FLAG_DEBUGGABLE]
     * set.
     *
     * @param context The [Context] for the package.
    ...
    /**
     * Check if the app associated with the `applicationInfo` has [ApplicationInfo.FLAG_DEBUGGABLE]
     * set.
     *
     * @param applicationInfo The [ApplicationInfo] for the package.
     * @return Returns `true` if app is debuggable, otherwise `false`.
    ...
    /**
     * Check if the app associated with the `context` has [ApplicationInfo.FLAG_EXTERNAL_STORAGE]
     * set.
     *
     * @param context The [Context] for the package.
    ...
    /**
     * Check if the app associated with the `applicationInfo` has [ApplicationInfo.FLAG_EXTERNAL_STORAGE]
     * set.
     *
     * @param applicationInfo The [ApplicationInfo] for the package.
     * @return Returns `true` if app is installed on external storage, otherwise `false`.
    ...
    fun isAppInstalledOnExternalStorage(applicationInfo: ApplicationInfo): Boolean {
        return (applicationInfo.flags and ApplicationInfo.FLAG_EXTERNAL_STORAGE) != 0
    }
    ...
    fun getProfileOwnerPackageNameForUser(context: Context): String? {
        val devicePolicyManager = context.getSystemService(Context.DEVICE_POLICY_SERVICE) as? DevicePolicyManager ?: return null
        ...
        if (activityManager != null) {
            ...
            if (processInfos != null) {
                for (i in 0 until processInfos.size) {
                    val processInfo = processInfos[i]
    ...
    fun setComponentState(
        ...
        var localToastString = toastString
        try {
            val packageManager = context.packageManager
            if (packageManager != null) {
                if (localToastString != null && alwaysShowToast) {
                    Logger.showToast(context, localToastString, true)
                    localToastString = null
                }
                ...
                if (localToastString != null) Logger.showToast(context, localToastString, true)
     ...
     * @return Returns `true` if disabled, `false` if not and `null` if failed to
     * get the state.
     */
    @JvmStatic
