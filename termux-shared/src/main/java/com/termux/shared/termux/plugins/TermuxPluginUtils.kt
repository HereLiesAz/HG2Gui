...
object TermuxPluginUtils {
    ...
    fun processPluginExecutionCommandResult(context: Context?, logTag: String?, executionCommand: ExecutionCommand?) {
        ...
        if (isPluginExecutionCommandWithPendingResult) {
            ...
            error = ResultSender.sendCommandResultData(context, tag, executionCommand.commandIdAndLabelLogString,
    ...
    fun processPluginExecutionCommandError(context: Context?, logTag: String?,
                                           executionCommand: ExecutionCommand?,
                                           forceNotification: Boolean) {
        var forceNotif = forceNotification
        ...
        if (!executionCommand.isStateFailed) {
            Logger.logWarn(tag, executionCommand.commandIdAndLabelLogString + ": Ignoring call to processPluginExecutionCommandError() since the execution command is not in ExecutionState.FAILED")
            return
        }
        ...
        sendPluginCommandErrorNotification(context, tag, null,
            ResultData.getErrorsListMinimalString(resultData),
            ExecutionCommand.getExecutionCommandMarkdownString(executionCommand),
            forceNotif, true, TermuxUtils.AppInfoMode.TERMUX_AND_CALLING_PACKAGE, true,
            executionCommand.resultConfig.resultPendingIntent?.creatorPackage)
    }
    ...
    fun sendPluginCommandErrorNotification(currentPackageContext: Context?, logTag: String?,
        ...
        val displayTitle = if (title == null || title.toString().isEmpty()) {
            TermuxConstants.TERMUX_APP_NAME + " Plugin Execution Command Error"
        } else {
            title
        }

        Logger.logDebug(tag, "Sending \"" + displayTitle + "\" notification.")
        ...
        val userActionName = UserAction.PLUGIN_EXECUTION_COMMAND.name

        val reportInfo = ReportInfo(userActionName, tag, displayTitle.toString())
        ...
        val builder = getPluginCommandErrorsNotificationBuilder(currentPackageContext, termuxPackageContext,
            displayTitle, notificationTextCharSequence, notificationTextCharSequence, contentIntent, deleteIntent,
