 ...
 * [TermuxConstants.TERMUX_PROPERTIES_PRIMARY_FILE_PATH] or
...
object TermuxPropertyConstants {
    ...
    const val KEY_DISABLE_FILE_SHARE_RECEIVER = "disable-file-share-receiver" // Default: "disable-file-share-receiver"

    /** Defines the key for whether file view receiver of the app is enabled. */
    const val KEY_DISABLE_FILE_VIEW_RECEIVER = "disable-file-view-receiver" // Default: "disable-file-view-receiver"

    /** Defines the key for whether hardware keyboard shortcuts are enabled. */
    const val KEY_DISABLE_HARDWARE_KEYBOARD_SHORTCUTS = "disable-hardware-keyboard-shortcuts" // Default: "disable-hardware-keyboard-shortcuts"

    /** Defines the key for whether a toast will be shown when user changes the terminal session */
    const val KEY_DISABLE_TERMINAL_SESSION_CHANGE_TOAST = "disable-terminal-session-change-toast" // Default: "disable-terminal-session-change-toast"
    ...
    const val KEY_ENFORCE_CHAR_BASED_INPUT = "enforce-char-based-input" // Default: "enforce-char-based-input"

    /** Defines the key for whether text for the extra keys buttons should be all capitalized automatically */
    const val KEY_EXTRA_KEYS_TEXT_ALL_CAPS = "extra-keys-text-all-caps" // Default: "extra-keys-text-all-caps"

    /** Defines the key for whether to hide soft keyboard when termux app is started */
    const val KEY_HIDE_SOFT_KEYBOARD_ON_STARTUP = "hide-soft-keyboard-on-startup" // Default: "hide-soft-keyboard-on-startup"

    /** Defines the key for whether the {@link TermuxAmSocketServer} should be run at app startup */
    const val KEY_RUN_TERMUX_AM_SOCKET_SERVER = "run-termux-am-socket-server" // Default: "run-termux-am-socket-server"

    /** Defines the key for whether url links in terminal transcript will automatically open on click or on tap */
    const val KEY_TERMINAL_ONCLICK_URL_OPEN = "terminal-onclick-url-open" // Default: "terminal-onclick-url-open"

    /** Defines the key for whether to use black UI */
    @Deprecated("")
    const val KEY_USE_BLACK_UI = "use-black-ui" // Default: "use-black-ui"

    /** Defines the key for whether to use ctrl space workaround to fix the issue where ctrl+space does not work on some ROMs */
    const val KEY_USE_CTRL_SPACE_WORKAROUND = "ctrl-space-workaround" // Default: "ctrl-space-workaround"

    /** Defines the key for whether to use fullscreen */
    const val KEY_USE_FULLSCREEN = "fullscreen" // Default: "fullscreen"
    ...
    const val KEY_BELL_BEHAVIOUR = "bell-character" // Default: "bell-character"
    ...
    const val KEY_TERMINAL_CURSOR_STYLE = "terminal-cursor-style" // Default: "terminal-cursor-style"
    ...
    const val IVALUE_TERMINAL_CURSOR_STYLE_BLOCK = TerminalEmulator.TERMINAL_CURSOR_STYLE_BLOCK
    const val IVALUE_TERMINAL_CURSOR_STYLE_UNDERLINE = TerminalEmulator.TERMINAL_CURSOR_STYLE_UNDERLINE
    const val IVALUE_TERMINAL_CURSOR_STYLE_BAR = TerminalEmulator.TERMINAL_CURSOR_STYLE_BAR
    const val DEFAULT_IVALUE_TERMINAL_CURSOR_STYLE = TerminalEmulator.DEFAULT_TERMINAL_CURSOR_STYLE
     ...
     */
    const val KEY_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT = "delete-tmpdir-files-older-than-x-days-on-exit" // Default: "delete-tmpdir-files-older-than-x-days-on-exit"
    ...
    const val KEY_TERMINAL_MARGIN_HORIZONTAL = "terminal-margin-horizontal" // Default: "terminal-margin-horizontal"
    ...
    const val KEY_TERMINAL_MARGIN_VERTICAL = "terminal-margin-vertical" // Default: "terminal-margin-vertical"
    ...
    const val KEY_TERMINAL_TRANSCRIPT_ROWS = "terminal-transcript-rows" // Default: "terminal-transcript-rows"
    const val IVALUE_TERMINAL_TRANSCRIPT_ROWS_MIN = TerminalEmulator.TERMINAL_TRANSCRIPT_ROWS_MIN
    const val IVALUE_TERMINAL_TRANSCRIPT_ROWS_MAX = TerminalEmulator.TERMINAL_TRANSCRIPT_ROWS_MAX
    const val DEFAULT_IVALUE_TERMINAL_TRANSCRIPT_ROWS = TerminalEmulator.DEFAULT_TERMINAL_TRANSCRIPT_ROWS

    /* float */

    /** Defines the key for the terminal toolbar height */
    const val KEY_TERMINAL_TOOLBAR_HEIGHT_SCALE_FACTOR = "terminal-toolbar-height" // Default: "terminal-toolbar-height"
    ...
    const val KEY_SHORTCUT_CREATE_SESSION = "shortcut.create-session" // Default: "shortcut.create-session"
    /** Defines the key for next session shortcut */
    const val KEY_SHORTCUT_NEXT_SESSION = "shortcut.next-session" // Default: "shortcut.next-session"
    /** Defines the key for previous session shortcut */
    const val KEY_SHORTCUT_PREVIOUS_SESSION = "shortcut.previous-session" // Default: "shortcut.previous-session"
    /** Defines the key for rename session shortcut */
    const val KEY_SHORTCUT_RENAME_SESSION = "shortcut.rename-session" // Default: "shortcut.rename-session"
    ...
    const val KEY_BACK_KEY_BEHAVIOUR = "back-key" // Default: "back-key"
    ...
    const val KEY_DEFAULT_WORKING_DIRECTORY = "default-working-directory" // Default: "default-working-directory"
    /** Defines the default working directory */
    const val DEFAULT_IVALUE_DEFAULT_WORKING_DIRECTORY = TermuxConstants.TERMUX_HOME_DIR_PATH

    /** Defines the key for extra keys */
    const val KEY_EXTRA_KEYS = "extra-keys" // Default: "extra-keys"
    //public static final String DEFAULT_IVALUE_EXTRA_KEYS = "[[ESC, TAB, CTRL, ALT, {key: '-', popup: '|'}, DOWN, UP]]"; // Single row
    const val DEFAULT_IVALUE_EXTRA_KEYS = "[['ESC','/',{key: '-', popup: '|'},'HOME','UP','END','PGUP'], ['TAB','CTRL','ALT','LEFT','DOWN','RIGHT','PGDN']]" // Double row

    /** Defines the key for extra keys style */
    const val KEY_EXTRA_KEYS_STYLE = "extra-keys-style" // Default: "extra-keys-style"
    const val DEFAULT_IVALUE_EXTRA_KEYS_STYLE = "default"

    /** Defines the key for {@link NightMode}. */
    const val KEY_NIGHT_MODE = "night-mode" // Default: "night-mode"

    @JvmField
    val IVALUE_NIGHT_MODE_TRUE = NightMode.TRUE.getName()
    @JvmField
    val IVALUE_NIGHT_MODE_FALSE = NightMode.FALSE.getName()
    @JvmField
    val IVALUE_NIGHT_MODE_SYSTEM = NightMode.SYSTEM.getName()
    @JvmField
    val DEFAULT_IVALUE_NIGHT_MODE = IVALUE_NIGHT_MODE_SYSTEM
    ...
    const val KEY_SOFT_KEYBOARD_TOGGLE_BEHAVIOUR = "soft-keyboard-toggle-behaviour" // Default: "soft-keyboard-toggle-behaviour"
    ...
    const val KEY_VOLUME_KEYS_BEHAVIOUR = "volume-keys" // Default: "volume-keys"
     ...
     * */
    @JvmField
    val TERMUX_APP_PROPERTIES_LIST: Set<String> = HashSet(listOf(
        /* boolean */
     ...
     */
    @JvmField
    val TERMUX_DEFAULT_FALSE_BOOLEAN_BEHAVIOUR_PROPERTIES_LIST: Set<String> = HashSet(listOf(
        KEY_DISABLE_FILE_SHARE_RECEIVER,
        KEY_DISABLE_FILE_VIEW_RECEIVER,
        KEY_DISABLE_HARDWARE_KEYBOARD_SHORTCUTS,
        KEY_DISABLE_TERMINAL_SESSION_CHANGE_TOAST,
     ...
     */
    @JvmField
    val TERMUX_DEFAULT_TRUE_BOOLEAN_BEHAVIOUR_PROPERTIES_LIST: Set<String> = HashSet(listOf(
        KEY_EXTRA_KEYS_TEXT_ALL_CAPS,
        KEY_RUN_TERMUX_AM_SOCKET_SERVER
    ))
     ...
     */
    @JvmField
    val TERMUX_DEFAULT_INVERETED_FALSE_BOOLEAN_BEHAVIOUR_PROPERTIES_LIST: Set<String> = HashSet()
     ...
     */
    @JvmField
    val TERMUX_DEFAULT_INVERETED_TRUE_BOOLEAN_BEHAVIOUR_PROPERTIES_LIST: Set<String> = HashSet()
