    ...
    public String exec(final ExecutePack pack) throws Exception {
        new StoppableThread() {
            ...
            public void run() {
                ...
                LocalBroadcastManager.getInstance(((MainPack) pack).androidContext.getApplicationContext()).sendBroadcast(new Intent(UIManager.ACTION_UPDATE_HINT));
            }
        }.start();

        LocalBroadcastManager.getInstance(((MainPack) pack).androidContext.getApplicationContext()).sendBroadcast(new Intent(UIManager.ACTION_NOROOT));
