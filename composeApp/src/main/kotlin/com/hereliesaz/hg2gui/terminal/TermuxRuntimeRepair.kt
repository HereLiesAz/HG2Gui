...
object TermuxRuntimeRepair {
    ...
    private fun writeAptConfig(prefix: File) {
        ...
        File(prefix, "var/cache/apt/archives/partial").mkdirs(); File(prefix, "var/lib/apt/lists/partial").mkdirs()
    ...
    private fun writeScriptWrappers(prefix: File, home: File) {
        home.mkdirs()
        val scripts = File(prefix, "bin").listFiles().orEmpty().filter { it.isFile && !Files.isSymbolicLink(it.toPath()) && !isElf(it) }.map { it.name }.sorted()
        File(home, WRAPPERS).writeText(buildString { appendLine("# Auto-generated by HG2Gui. Do not edit."); scripts.forEach { appendLine("$it() ( source \"\$PREFIX/bin/$it\" \"\$@\" )") } })
        val profile = File(home, ".bash_profile"); val existing = if (profile.exists()) profile.readText() else ""
        if (PROFILE_LINE !in existing) profile.appendText((if (existing.isNotEmpty() && !existing.endsWith('\n')) "\n" else "") + PROFILE_LINE + "\n")
    }

    private fun isElf(file: File): Boolean = runCatching { file.inputStream().use { input -> val m = ByteArray(4); input.read(m) == 4 && m[0].toInt() == 0x7f && m[1] == 'E'.code.toByte() && m[2] == 'L'.code.toByte() && m[3] == 'F'.code.toByte() } }.getOrDefault(false)
}
